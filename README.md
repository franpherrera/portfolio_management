...
Example usage in `portfolio.py`:
This alternative use case considers the scenario in which we already have some stock shares and we want to add a new one along with new target allocations. In order to use this we must declare the new stock ticker and how the new target allocations are composed.
